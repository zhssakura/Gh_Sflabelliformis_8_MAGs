...
library(sybil)
library(data.table)
mod <- readRDS(paste(getwd,"/07_adapt_addReact_R/04_Rscp/OTU08_MOM_20210330pm1/20210330pm1_04.medium_Jo_no_photo_STY_Merged_OTU08_arc__04_Rscp_NOcitrate_sulfate/STY_Merged_OTU08_MOM.RDS",sep = ''))
...
sol.dt[grepl("rxn00433_Shan_OTU08_20210330|rxn00598_Shan_OTU08_20210330|rxn00820_Shan_OTU08_20210330", rxn)] # methanophenanzine-using reactions
# ```

# Lets consider another methanogenic archaeon which does not possess cytochromes.
...
#   ```R
# mod <- readRDS("GCF_000008645.1_ASM864v1_genomic.RDS")
mtf2 <- optimizeProb(mod, algorithm="mtf"); print(mtf2)
ex2 <- findExchReact(mod)
ex2.dt <- data.table(ex=ex2@react_id, met=mod@met_name[ex2@met_pos], flux=mtf2@fluxdist@fluxes[ex2@react_pos])
...
#   ```R
sol2.dt <- data.table(rxn=mod@react_id, name=mod@react_name, flux=mtf2@fluxdist@fluxes[1:mod@react_num])
...
mtf2@fluxdist@fluxes[which(mod@obj_coef!=0)] / mtf2@fluxdist@fluxes[grep("EX_cpd01024", mod@react_id)] * 1000 # yield_ch4 = 3.33 / mol
# ```
