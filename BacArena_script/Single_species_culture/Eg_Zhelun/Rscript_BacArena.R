...
library(BacArena)
library(parallel)
library(cplexAPI) # add @2021-09-08
SYBIL_SETTINGS("SOLVER", "cplexAPI") # add @2021-09-08
######################## argument ########################
option_list = list(
  optparse::make_option(c("-o", "--otu_name"),                 type="character",      default='test',       help="OTU_name, defalt: test"),
  optparse::make_option(c("-m", "--infile_my_model"),          type="character",      default='/srv/scratch/z5095298/sponge_modeling/GapSeq_v20210429_STY8taxa_gapseq_v0429_MKSdb_Diet_sw_lALL_b_uncon/OTU06_renamed/20210515_STY_Merged_OTU06_mineral_sw_3_NoH2S/STY_Merged_OTU06.RDS',            
                        help="Directory of input final gs model, defalt: /srv/scratch/z5095298/sponge_modeling/GapSeq_v20210429_STY8taxa_gapseq_v0429_MKSdb_Diet_sw_lALL_b_uncon/OTU06_renamed/20210515_STY_Merged_OTU06_mineral_sw_3_NoH2S/STY_Merged_OTU06.RDS"),
...
simlist <- parLapply(cl, 1:replicates, function(i){
  print("====================================================================")
  print("============================= Autotrophs - setAllExInf=TRUE,limit_growth = T =======================================")
  print("====================================================================")
  bacterium_OTU06 <- BacArena::Bac(my_model,lyse = F,deathrate=death_r,maxweight = 1, minweight=0.125,cellweight_mean = 0.5,cellweight_sd = 0.25, growtype='exponential',setAllExInf=TRUE,limit_growth = T)
  arena <- BacArena::Arena(n=grid_no,m=grid_no)
  inocc <- arena@n * arena@m * Inocc_no # 5% inocculum
  arena <- BacArena::addOrg(object = arena, specI = bacterium_OTU06, amount = inocc*1) # amount can be added by fraction.
...
saveRDS(simlist, file = paste(getwd,'/',new_folder,'/BacArena_STY_',otu_name,'.RDS',sep = ''))

# ############################ Download RDS file and run on MacOS ############################
getwd <- getwd()
getwd
simulation_loop <- readRDS(paste(getwd,'/',new_folder,'/BacArena_STY_',otu_name,'.RDS',sep = ''))
...
pdf(paste(getwd,'/',new_folder,'/001_plotGrowthCurve_biomass_STY_',otu_name,'.pdf',sep = ''),width = 5, height = 4)
plotGrowthCurve(simulation_loop,use_biomass = T)
dev.off()
use_biomass <- plotGrowthCurve(simulation_loop,use_biomass = T,ret_data = T)
write.csv(use_biomass, file = paste(getwd,'/',new_folder,'/001_plotGrowthCurve_biomass_STY_',otu_name,'.csv',sep = ''))
#
pdf(paste(getwd,'/',new_folder,'/002_plotGrowthCurve_number_STY_',otu_name,'.pdf',sep = ''),width = 5, height = 4)
plotGrowthCurve(simulation_loop,use_biomass = F)
dev.off()
Nouse_biomass <- plotGrowthCurve(simulation_loop,use_biomass = F,ret_data = T)
write.csv(Nouse_biomass, file = paste(getwd,'/',new_folder,'/002_plotGrowthCurve_number_STY_',otu_name,'.csv',sep = ''))
